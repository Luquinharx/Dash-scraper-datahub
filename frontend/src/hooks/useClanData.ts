...
export function useClanData() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const parseCollectedAtTs = (value: unknown): number => {
        if (!value) return 0;
        const parsed = Date.parse(String(value));
        return Number.isFinite(parsed) ? parsed : 0;
      };
      const fetchJson = async (path: string, cacheBust: number): Promise<any> => {
        const hasQuery = path.includes('?');
        const url = `${FIREBASE_URL}${path}${hasQuery ? '&' : '?'}_cb=${cacheBust}`;
      ...
      const [profilesJson, dailyJson, todayDailyJson] = await Promise.all([
        fetchJson('/profiles.json', cacheBust),
        fetchJson(`/daily.json?orderBy=%22$key%22&endAt=${encodeURIComponent(`"${yesterdayStr}"`)}&limitToLast=7`, cacheBust),
        fetchJson(`/daily/${todayStr}.json`, cacheBust),
      ]);

      const profiles = profilesJson || {};
      const dailyData = dailyJson || {};
      const todayDailyData = todayDailyJson || {};
      ...
      let globalCollectedTs = 0;

      users.forEach((u) => {
        const val = profiles[u];
        if (!val) return;

        const collectedAtRaw = val.collected_at;
        const collectedAtTs = parseCollectedAtTs(collectedAtRaw);
        if (collectedAtTs > globalCollectedTs) {
          globalCollectedTs = collectedAtTs;
          globalCollectedAt = String(collectedAtRaw || '');
        } else if (!globalCollectedAt && collectedAtRaw) {
          globalCollectedAt = String(collectedAtRaw);
        }
