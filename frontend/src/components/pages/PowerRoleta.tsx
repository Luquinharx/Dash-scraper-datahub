...
export default function PowerRoleta() {
  ...
  const weeklyLoot = Number(stats?.weekly_loots || 0);
  const isQualified = weeklyLoot >= 800;
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const spinsData = (snap.val() || {}) as Record<string, { userId?: string; premio?: string; data?: unknown; entregue?: boolean }>;

        let count = 0;
        const list: { prize: string; date: string; claimed: boolean }[] = [];
        const startTs = startOfPrizeWeek.getTime();

        Object.values(spinsData).forEach((entry) => {
          ...
          list.push({
            prize: entry.premio || '-',
            date: formatDateTimePtBR(entry.data),
            claimed: !!entry.entregue,
          });
        });
  ...
  const spinWheel = useCallback(async () => {
    ...
    try {
      await push(ref(rtdb, 'power_roletas'), {
      ...
      if (!hasWeeklyAvailable && powerSpins > 0) {
        await runTransaction(ref(rtdb, `usuarios/${profile.userId}/powerSpins`), (current) => {
  ...
  return (
    <div className="w-full text-zinc-200 font-sans selection:bg-red-900/30 pt-4">
      <div className="w-full mx-auto space-y-10 animate-in fade-in duration-700">
        ...
        <header className="flex flex-col sm:flex-row items-center justify-between border-b border-white/10 pb-6 gap-4">
          <div className="flex items-center gap-5">
            <div className="p-4 bg-white/[0.04] rounded-sm text-zinc-300 border border-white/10">
              <Zap className="w-8 h-8" />
            </div>
            <div>
              <h1 className="text-3xl md:text-5xl font-semibold text-white uppercase font-sans">
                <span className="text-red-600">Blood</span> Wheel
              </h1>
              <p className="text-zinc-400 font-medium tracking-[0.12em] uppercase mt-2 text-sm flex items-center gap-2">
                <Info className="w-4 h-4"/> ONE SPIN PER WEEK • QUALIFICATION: 800+ WEEKLY LOOT
              </p>
            </div>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="soft-card rounded-sm p-5 text-center relative overflow-hidden group">
            <div className="absolute top-0 left-0 w-1 h-full bg-red-600/50"></div>
            <p className="text-xs text-stone-500 uppercase tracking-widest font-black">Weekly Loot</p>
          ...
          <div className="soft-card rounded-sm p-5 text-center relative overflow-hidden">
             <div className="absolute top-0 left-0 w-1 h-full bg-red-600/50"></div>
            <p className="text-xs text-zinc-500 uppercase tracking-widest font-black">Goal</p>
        ...
        <div className="flex flex-col items-center gap-10 py-20 bg-gradient-to-b from-zinc-950 to-black rounded-sm border border-white/10 relative overflow-hidden shadow-2xl">
            {/* Background Studio Lights */}
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-red-950/10 via-black/90 to-black pointer-events-none"></div>

            {/* Turn Count Display */}
            <div className="z-10 bg-black/80 px-8 py-3 rounded-sm border border-white/10 backdrop-blur-md shadow-[0_0_30px_rgba(0,0,0,0.8)]">
                <span className="text-zinc-300 font-bold uppercase tracking-[0.12em] text-sm flex items-center gap-2">
                ...
                <div className="w-full h-full rounded-full relative bg-gradient-to-tr from-yellow-700 via-yellow-400 to-yellow-600 shadow-[0_30px_60px_rgba(0,0,0,0.9),inset_0_5px_15px_rgba(255,255,255,0.4)] border border-yellow-300/30 p-4 sm:p-6 flex items-center justify-center">
                    ...
                    >
                        ...
                        <div className="absolute inset-0 m-auto w-20 h-20 sm:w-28 sm:h-28 rounded-full bg-gradient-to-tr from-stone-900 to-stone-700 shadow-[0_0_30px_rgba(0,0,0,1)] border-[6px] border-stone-800 flex items-center justify-center z-20">
                            <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-gradient-to-br from-zinc-900 to-red-700 shadow-[inset_0_2px_10px_rgba(0,0,0,0.8)] flex items-center justify-center border-[3px] border-zinc-950/80">
            ...
            {/* Spin Control Button */}
             <button
                ...
                className={cn(
                  ...
                  spinning
                    ...
                    : availableSpins > 0
                      ? "bg-white text-black hover:bg-zinc-200 border-zinc-400"
             ...
             {result && !spinning && (
                <div className="z-20 animate-in fade-in zoom-in duration-500 mt-8 text-center bg-zinc-950/80 px-12 py-8 rounded-sm border border-white/10 backdrop-blur-xl shadow-[0_20px_50px_rgba(0,0,0,0.8)] min-w-[300px]">
                    <div className="text-zinc-300 font-sans font-bold uppercase text-xs tracking-[0.3em] mb-4 flex items-center justify-center gap-2">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mt-16 w-full max-w-6xl px-6 opacity-90 z-10">
                {config.prizes.map((p) => (
                    <div key={p.id} className="flex flex-col items-center justify-center p-6 rounded-sm bg-black/40 border border-white/10 backdrop-blur-xl group hover:border-white/20 hover:bg-white/[0.04] transition-all duration-500">
        ...
        {history.length > 0 && (
          <div className="surface-panel rounded-sm overflow-hidden">
            <div className="px-8 py-6 border-b border-white/10 bg-black/40 flex items-center gap-3">
              <Zap className="w-5 h-5 text-red-500" />
              <h2 className="text-xl font-semibold text-white tracking-[0.12em] uppercase">Spin History</h2>
            </div>
