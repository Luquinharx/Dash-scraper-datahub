...
import { createUserWithEmailAndPassword, getAuth, sendPasswordResetEmail, updatePassword } from 'firebase/auth';
...
export default function GerenciarUsuarios() {
  ...
  const [editForm, setEditForm] = useState({ nick: '', discord: '', cargo: '', nickJogo: '', extraSpins: 0, powerSpins: 0, newPassword: '' });
  ...
  async function saveEdit(userToSave: UserProfile & { docId: string }) {
    try {
      setMemberActionError('');
      setMemberActionSuccess('');

      await update(ref(rtdb, `usuarios/${userToSave.docId}`), {
        nick: editForm.nick,
      ...
      const nextPassword = editForm.newPassword.trim();
      if (nextPassword) {
        if (nextPassword.length < 6) {
          setMemberActionError('A nova senha deve ter pelo menos 6 caracteres.');
          return;
        }

        if (auth.currentUser?.uid === userToSave.userId) {
          try {
            await updatePassword(auth.currentUser, nextPassword);
            setMemberActionSuccess(`Senha do usuário "${userToSave.nick}" alterada com sucesso.`);
          } catch (passwordError: any) {
            const code = passwordError?.code || '';
            if (code === 'auth/requires-recent-login') {
              await sendPasswordResetEmail(auth, userToSave.email);
              setMemberActionSuccess(`Sessão antiga para troca direta. Link de redefinição enviado para ${userToSave.email}.`);
            } else {
              throw passwordError;
            }
          }
          ...
          await sendPasswordResetEmail(auth, userToSave.email);
          setMemberActionSuccess(`Troca direta de senha de outro usuário não é permitida no client SDK. Link de redefinição enviado para ${userToSave.email}.`);
        }
      }

      setEditingId(null);
      setEditForm({ nick: '', discord: '', cargo: '', nickJogo: '', extraSpins: 0, powerSpins: 0, newPassword: '' });
      await loadAll();
    } catch (err) {
      console.error('Error editing:', err);
      setMemberActionError('Falha ao salvar alterações do usuário.');
    }
  }
...
<div className="flex gap-2 bg-stone-900/50 p-1 rounded-sm border border-white/5 overflow-x-auto">
        ...
        {activeTab === 'members' && (
            ...
            {memberActionError && (
                <div className="p-4 bg-red-950/30 border border-red-900/50 text-red-400 text-xs font-mono uppercase tracking-wider">
                    {memberActionError}
                </div>
            )}
            {memberActionSuccess && (
                <div className="p-4 bg-emerald-950/30 border border-emerald-900/50 text-emerald-400 text-xs font-mono uppercase tracking-wider">
                    {memberActionSuccess}
                </div>
            )}
            ...
            <div className="bg-stone-950/50 rounded-sm border border-white/10 overflow-hidden backdrop-blur-sm">
                ...
                <table className="w-full text-sm text-left font-mono">
                    ...
                    <tr>
                        ...
                        <th className="px-6 py-4 font-normal whitespace-nowrap">Password</th>
                        ...
                        {editingId === u.docId ? (
                            ...
                            <td className="px-6 py-3">
                                <input
                                    ...
                                    className="w-36 px-2 py-1 bg-black border border-white/20 rounded-sm text-white text-xs"
                            ...
                            <td className="px-6 py-3 text-center">
                                ...
                                <button onClick={() => saveEdit(u)} className="text-emerald-500 hover:text-emerald-400"><Save className="w-4 h-4" /></button>
                            ...
                            <td className="px-6 py-3 text-center text-stone-700 text-xs">••••••</td>
                            <td className="px-6 py-3 text-center">
